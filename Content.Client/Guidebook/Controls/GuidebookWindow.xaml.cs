...
public sealed partial class GuidebookWindow : FancyWindow, ILinkClickHandler, IAnchorClickHandler
{
        ...
        HashSet<ProtoId<GuideEntryPrototype>> addedEntries)
    {
        ...
        if (entry.RequiredCulture is { } requiredCulture)
        {
            var current = _cfg.GetCVar(CVars.LocCultureName);
            if (!current.StartsWith(requiredCulture, StringComparison.OrdinalIgnoreCase))
                return null;
        }
