...
public sealed class SmartEquipSystem : EntitySystem
{
    ...
    private void HandleSmartEquip(ICommonSession? session, string equipmentSlot)
    {
        ...
        if (equipmentSlot == "suitstorage" && handItem == null)
        {
            if (!_inventory.CanUnequip(uid, equipmentSlot, out var suitReason))
            {
                _popup.PopupClient(Loc.GetString(suitReason), uid, uid);
                return;
            }
