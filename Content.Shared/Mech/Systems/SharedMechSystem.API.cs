...
{
    private const float MinimumUsableEnergy = 0.01f;

    private static float GetMinimumUsableEnergy(float maxCharge)
    {
        return MathF.Max(MinimumUsableEnergy, maxCharge * 0.01f);
    }
    ...
    public bool TryChangeEnergy(Entity<MechComponent?> ent, FixedPoint2 delta)
    {
        ...
        RefreshEnergyDependentState(ent);

        return true;
    }

    /// <summary>
    /// Returns whether the mech has enough usable power for a powered action.
    /// Tiny floating point residue is treated as empty so actions cannot run forever on near-zero charge.
    /// </summary>
    [PublicAPI]
    public bool HasUsableEnergy(Entity<MechComponent?> ent, float minimum = MinimumUsableEnergy)
    {
        if (!Resolve(ent, ref ent.Comp, false))
            return false;

        if (ent.Comp.Broken || ent.Comp.Integrity <= 0)
            return false;

        if (!_powerCell.TryGetBatteryFromSlot(ent.Owner, out var battery))
            return false;

        minimum = MathF.Max(minimum, GetMinimumUsableEnergy(battery.Value.Comp.MaxCharge));
        return _battery.GetCharge(battery.Value.AsNullable()) >= minimum;
    }

    /// <summary>
    /// Refreshes movement blockers and stops the walk animation if the mech no longer has usable power.
    /// </summary>
    [PublicAPI]
    public void RefreshEnergyDependentState(Entity<MechComponent?> ent)
    {
        if (!Resolve(ent, ref ent.Comp, false))
            return;
    ...
    public void UpdateBatteryAlert(Entity<MechComponent?> ent)
    {
        ...
        var chargeRatio = maxCharge > 0f
            ? Math.Clamp(charge / maxCharge, 0f, 1f)
            : 0f;

        var chargePercent = charge < GetMinimumUsableEnergy(maxCharge)
            ? (short)0
            : (short)Math.Clamp(MathF.Ceiling(chargeRatio * 10f), 1f, 10f);
