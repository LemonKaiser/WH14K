...
public abstract partial class SharedMechSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<MechEquipmentComponent, GunShotEvent>(OnMechEquipmentGunShot);
        ...
        SubscribeLocalEvent<MechComponent, ChargeChangedEvent>(OnMechBatteryChargeChanged);
        SubscribeLocalEvent<MechComponent, SpriteMoveEvent>(OnMechSpriteMove, after: [typeof(SharedSpriteMovementSystem)]);
    ...
    private void OnMapInit(Entity<MechComponent> ent, ref MapInitEvent args)
    {
        ...
        {
            var equipmentEnt = PredictedSpawnAttachedTo(equipment, xform.Coordinates);
            InsertEquipment(ent.AsNullable(), equipmentEnt);
        }

        foreach (var module in ent.Comp.StartingModules)
        {
            var equipmentEnt = PredictedSpawnAttachedTo(module, xform.Coordinates);
            InsertEquipment(ent.AsNullable(), equipmentEnt);
        }

        ent.Comp.Integrity = ent.Comp.MaxIntegrity;
        ent.Comp.Airtight = ent.Comp.CanAirtight;
    ...
    private void SetupUser(Entity<MechComponent> mechEnt, EntityUid pilot)
    {
        ...
        RemoveDuplicateLightAction(mechEnt.Owner);
        ManageVirtualItems(mechEnt, pilot, create: true);
    }

    private void RemoveDuplicateLightAction(EntityUid mechUid)
    {
        if (!TryComp<HandheldLightComponent>(mechUid, out var light)
            || light.SelfToggleActionEntity == null)
            return;

        _actions.RemoveAction(light.SelfToggleActionEntity);
    }
    ...
    private void OnMechCanMoveEvent(Entity<MechComponent> ent, ref UpdateCanMoveEvent args)
    {
        ...
        if (!HasUsableEnergy(ent.AsNullable()))
        {
            args.Cancel();
            return;
        }
    ...
    private void OnBatteryChanged(Entity<MechComponent> ent, ref PowerCellChangedEvent args)
    {
        ...
        UpdateMechUi(ent.Owner);
        UpdateBatteryAlert(ent.AsNullable());
        RefreshEnergyDependentState(ent.AsNullable());
    }

    private void OnMechBatteryChargeChanged(Entity<MechComponent> ent, ref ChargeChangedEvent args)
    {
        // Direct battery consumers such as mech lights bypass TryChangeEnergy, so keep the mech HUD in sync.
        UpdateMechUi(ent.Owner);
        UpdateBatteryAlert(ent.AsNullable());
        RefreshEnergyDependentState(ent.AsNullable());
    }

    private void OnMechSpriteMove(Entity<MechComponent> ent, ref SpriteMoveEvent args)
    {
        if (!args.IsMoving || HasUsableEnergy(ent.AsNullable()))
            return;

        if (!TryComp<SpriteMovementComponent>(ent.Owner, out var spriteMovement) || !spriteMovement.IsMoving)
            return;

        spriteMovement.IsMoving = false;
    ...
    private void OnBeingGibbed(Entity<MechComponent> ent, ref BeingGibbedEvent args)
    {
        ...
        if (ent.Comp.PilotSlot.ContainedEntity != null)
            args.Giblets.Add(ent.Comp.PilotSlot.ContainedEntity.Value);
    ...
    private bool IsInstalledMechEquipmentPowered(Entity<MechEquipmentComponent> ent, float requiredEnergy = 0f)
    {
        if (ent.Comp.EquipmentOwner is not { } mechUid)
            return true;

        return TryComp<MechComponent>(mechUid, out var mech)
               && HasUsableEnergy((mechUid, mech), requiredEnergy);
    }

    private float GetMechEquipmentShotEnergyCost(EntityUid equipment, int shots = 1)
    {
        var shotCount = Math.Max(shots, 1);

        if (TryComp<BatteryAmmoProviderComponent>(equipment, out var batteryAmmo))
            return MathF.Max(0f, batteryAmmo.FireCost) * shotCount;
    ...
    private void OnMechEquipmentShotAttempt(Entity<MechEquipmentComponent> ent, ref ShotAttemptedEvent args)
    {
        ...
        var shotEnergyCost = GetMechEquipmentShotEnergyCost(ent.Owner);
        if (!IsInstalledMechEquipmentPowered(ent, shotEnergyCost))
            args.Cancel();
    }

    private void OnMechEquipmentGunShot(Entity<MechEquipmentComponent> ent, ref GunShotEvent args)
    {
        if (_net.IsClient)
            return;

        if (ent.Comp.EquipmentOwner is not { } mechUid || !TryComp<MechComponent>(mechUid, out var mech))
            return;

        var shotEnergyCost = GetMechEquipmentShotEnergyCost(ent.Owner, args.Ammo.Count);
        if (shotEnergyCost <= 0f)
            return;

        if (HasUsableEnergy((mechUid, mech), shotEnergyCost)
            && TryChangeEnergy((mechUid, mech), -FixedPoint2.New(shotEnergyCost)))
        {
            return;
        }
        ...
        UpdateBatteryAlert((mechUid, mech));
        RefreshEnergyDependentState((mechUid, mech));
    }

    private void OnMechEquipmentMeleeAttempt(Entity<MechEquipmentComponent> ent, ref AttemptMeleeEvent args)
    {
        args.Cancelled = !IsMechEquipmentUsableFromHands(ent) || !IsInstalledMechEquipmentPowered(ent);
    }
    ...
    private void OnMechEquipmentGettingUsedAttempt(Entity<MechEquipmentComponent> ent, ref GettingUsedAttemptEvent args)
    {
        ...
        if (!IsInstalledMechEquipmentPowered(ent))
            args.Cancel();
    }
    ...
    private void OnMechEquipmentUiOpenAttempt(Entity<MechEquipmentComponent> ent, ref ActivatableUIOpenAttemptEvent args)
    {
        ...
        if (ent.Comp.EquipmentOwner == null || !IsInstalledMechEquipmentPowered(ent))
            args.Cancel();
    }
    ...
    {
        args.CanAttack = TryComp<MechComponent>(ent.Comp.Mech, out var mech)
                         && HasUsableEnergy((ent.Comp.Mech, mech));
    }
    ...
    private void OnGetMeleeWeapon(Entity<MechPilotComponent> ent, ref GetMeleeWeaponEvent args)
    {
        ...
        if (!HasUsableEnergy((ent.Comp.Mech, mech)))
        {
            args.Handled = true;
            return;
        }
    ...
    private void OnGetShootingEntity(Entity<MechPilotComponent> ent, ref GetShootingEntityEvent args)
    {
        ...
        if (!TryComp<MechComponent>(ent.Comp.Mech, out var mech) || !HasUsableEnergy((ent.Comp.Mech, mech)))
            return;
